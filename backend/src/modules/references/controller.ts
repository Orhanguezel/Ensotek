...
/** LIST (public) */
export const listReferencesPublic: RouteHandler<{
...
/** GET BY ID (public) */
export const getReferencePublic: RouteHandler<{
  Params: { id: string };
}> = async (req, reply) => {
  const locale: Locale = (req as any).locale ?? DEFAULT_LOCALE;
...
/** GET BY SLUG (public) */
export const getReferenceBySlugPublic: RouteHandler<{
  Params: { slug: string };
}> = async (req, reply) => {
  const locale: Locale = (req as any).locale ?? DEFAULT_LOCALE;
...
export const listReferenceImagesPublic: RouteHandler<{
  Params: { id: string };
}> = async (req, reply) => {
  const locale: Locale = (req as any).locale ?? DEFAULT_LOCALE;
