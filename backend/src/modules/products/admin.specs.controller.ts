...
export const adminCreateProductSpec: RouteHandler = async (req, reply) => {
  ...
  try {
    ...
    const body = productSpecCreateSchema.parse({
      ...(req.body || {}),
      product_id: id,
    });
    const row = {
...
export const adminUpdateProductSpec: RouteHandler = async (req, reply) => {
  ...
  try {
    const patch = productSpecUpdateSchema.parse({
      ...(req.body || {}),
      product_id: id,
    });
    await db
...
export const adminReplaceSpecs: RouteHandler = async (req, reply) => {
  const { id } = req.params as { id: string };
  const items = Array.isArray((req.body as any)?.specs)
    ? (req.body as any).specs
    : Array.isArray((req.body as any)?.items)
