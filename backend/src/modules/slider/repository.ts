...
export async function repoListPublic(q: PublicListQuery): Promise<RowWithAsset[]> {
  ...
  // locale zorunlu (schema default "de")
  ...
  const rows = await db
    ...
    .from(slider)
    .innerJoin(sliderI18n, and(eq(sliderI18n.sliderId, slider.id)))
...
export async function repoGetBySlug(slugStr: string, locale: string): Promise<RowWithAsset | null> {
  const storage = await getStorageSettings();
  const bases: StorageBases = {
    cdnPublicBase: storage.cdnPublicBase,
    publicApiBase: storage.publicApiBase,
...
export async function repoListAdmin(q: AdminListQuery): Promise<RowWithAsset[]> {
  ...
  const baseQuery = db
    ...
    .from(slider)
    .innerJoin(sliderI18n, eq(sliderI18n.sliderId, slider.id))
    .leftJoin(storageAssets, eq(slider.image_asset_id, storageAssets.id));

  const rowsQuery = conds.length ? baseQuery.where(and(...conds)) : baseQuery;
 ...
 */
export async function repoGetById(id: number, locale?: string): Promise<RowWithAsset | null> {
  const storage = await getStorageSettings();
  const bases: StorageBases = {
    cdnPublicBase: storage.cdnPublicBase,
    publicApiBase: storage.publicApiBase,
  ...
  if (requestedLocale) {
    const rows = await db
      ...
      .innerJoin(
        sliderI18n,
        and(eq(sliderI18n.sliderId, slider.id), eq(sliderI18n.locale, requestedLocale)),
      )
      .leftJoin(storageAssets, eq(slider.image_asset_id, storageAssets.id))
...
export async function repoCreate(b: CreateBody): Promise<RowWithAsset> {
  ...
  const rows = await db
    ...
    .from(slider)
    .innerJoin(sliderI18n, and(eq(sliderI18n.sliderId, slider.id), eq(sliderI18n.locale, locale)))
    .leftJoin(storageAssets, eq(slider.image_asset_id, storageAssets.id))
...
export async function repoUpdate(id: number, b: UpdateBody): Promise<RowWithAsset | null> {
  ...
  if (i18nKeys.length) {
    ...
    if (existing) {
      ...
      const name = b.name ?? ''; // boş da olsa DB kabul eder ama pratikte FE full form gönderiyor
