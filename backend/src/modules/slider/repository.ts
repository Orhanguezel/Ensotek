...
export async function repoListPublic(
  ...
  const conds = [eq(slider.is_active, 1 as const), eq(slider.locale, q.locale)];
  if (q.q) {
    const s = `%${q.q.trim()}%`;
...
export async function repoGetBySlug(
  ...
  const rows = await db
    ...
    .from(slider)
    .leftJoin(storageAssets, eq(slider.image_asset_id, storageAssets.id))
    .where(and(eq(slider.slug, slugStr), eq(slider.locale, locale)))
    .limit(1);
...
export async function repoListAdmin(
  ...
  if (q.locale) {
    conds.push(eq(slider.locale, q.locale));
  }
  if (q.q) {
    const s = `%${q.q.trim()}%`;
    conds.push(like(slider.name, s));
...
export async function repoCreate(b: CreateBody): Promise<RowWithAsset> {
  ...
  const toInsert: Partial<SliderRow> = {
    uuid: randomUUID(),
    locale: b.locale ?? "tr",
    name: b.name,
    slug: baseSlug,
  ...
  await db.insert(slider).values(toInsert as any);
  const row = await repoGetBySlug(
    toInsert.slug as string,
    toInsert.locale as string,
  );
...
/** ✅ asset_id verilirse URL/asset_id set; null/undefined ⇒ temizle */
