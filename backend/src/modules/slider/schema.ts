 ...
 * locale destekli, image_url (legacy) + image_asset_id (storage) birlikte tutulur.
 */
export const slider = mysqlTable(
  "slider",
  {
    ...
    featured: tinyint("featured", { unsigned: true })
      .notNull()
      .default(0),
    is_active: tinyint("is_active", { unsigned: true })
      .notNull()
      .default(1),

    display_order: int("display_order", { unsigned: true })
      .notNull()
      .default(0),
  ...
  (t) => ({
    uniq_slug_locale: uniqueIndex("uniq_slider_slug_locale").on(
      t.slug,
      t.locale,
    ),
    ...
    idx_locale: index("idx_slider_locale").on(t.locale),
