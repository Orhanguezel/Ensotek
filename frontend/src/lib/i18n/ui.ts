...
import { useSettingsMap } from "@/lib/settings/hooks";
...
export const UI_FALLBACK_EN = {
  ...
  ui_newsletter_desc: "Get product updates and articles once or twice a month. No spam.",
 ...
 */
export function useUIStrings<T extends KeysArray>(keys: T, locale?: SupportedLocale) {
  const { map } = useSettingsMap(keys as unknown as string[], locale);
