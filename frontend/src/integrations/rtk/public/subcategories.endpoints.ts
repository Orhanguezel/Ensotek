...
import { normalizeSubCategory } from '@/integrations/types';
...
export const subCategoriesApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    /*  GET /sub-categories                                      */
    /* --------------------------------------------------------- */
    listSubCategories: build.query<SubCategoryDto[], SubCategoryListQueryParams | void>({
      query: (params) => {
    ...
    getSubCategoryById: build.query<SubCategoryDto, string>({
      ...
      providesTags: (_res, _err, id) => [{ type: 'SubCategories' as const, id }],
    }),
