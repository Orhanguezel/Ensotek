...
export const supportAdminApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    updateSupportTicketAdmin: builder.mutation<AdminSupportTicketDto, UpdateTicketArgs>({
      query: ({ id, patch }) => ({
        url: `/admin/support_tickets/${encodeURIComponent(id)}`,
        method: 'PATCH',
        body: patch,
    ...
    // POST /admin/support_tickets/:id/:action
    //   action = "close" | "reopen"
    toggleSupportTicketAdmin: builder.mutation<AdminSupportTicketDto, ToggleTicketArgs>({
      query: ({ id, action }) => ({
        url: `/admin/support_tickets/${encodeURIComponent(id)}/${encodeURIComponent(action)}`,
        method: 'POST',
      }),
    }),
  }),
