...
} from "@/integrations/rtk/endpoints/admin/newsletter_admin.endpoints";
...
const NewsletterAdminPage: React.FC = () => {
  ...
  const [formState, setFormState] = useState<NewsletterFormState | null>(
    null,
  );
  const [showModal, setShowModal] = useState(false);

  const openEditModal = (item: NewsletterAdminDto) => {
    let metaStr = "";
    try {
      metaStr =
        item.meta == null
  ...
  const handleSaveForm = async () => {
    ...
    if (formState.metaJson.trim().length > 0) {
      try {
        ...
        toast.error(
          "Meta alanı geçerli bir JSON olmalıdır. Lütfen kontrol et.",
        );
        return;
      }
    }
    ...
    const patch = {
      ...
      locale: formState.locale.trim() || null,
      meta: metaObj,
    };
  ...
  const handleDelete = async (item: NewsletterAdminDto) => {
    if (
      !window.confirm(
        `"${item.email}" aboneliğini silmek üzeresin. Devam etmek istiyor musun?`,
      )
    ) {
      return;
    }
    ...
    try {
      await deleteNewsletter(item.id).unwrap();
  ...
  const handleToggleVerified = async (
    ...
    try {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.id === item.id ? { ...r, is_verified: value } : r,
        ),
      );
    } catch (err: any) {
      const msg =
  ...
  const handleToggleSubscribed = async (
    ...
    try {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.id === item.id ? { ...r, is_subscribed: value } : r,
        ),
      );
    } catch (err: any) {
      const msg =
                ...
                <div className="modal-body">
                  ...
                  <div className="mt-3">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        handleFormChange("metaJson", e.target.value)
                      }
                    />
                    <div className="form-text small">
