...
// Ensotek – Admin Sparepart Create/Edit Page (module_key=sparepart)
...
const AdminSparepartDetailPage: NextPage = () => {
  ...
  const handleCancel = () => {
    void router.push('/admin/sparepart');
  };
  ...
  const handleSubmit = async (values: ProductFormValues) => {
    try {
      ...
      if (isCreateMode) {
        ...
        toast.success('Yedek parça başarıyla oluşturuldu.');

        void router.replace(`/admin/sparepart/${encodeURIComponent(created.id)}`);
      } else {
        if (!product) {
        ...
        toast.success('Yedek parça güncellendi.');
  ...
  const pageTitle = isCreateMode
    ...
    : product?.title || 'Yedek Parça Düzenle';
  ...
  if (!isCreateMode && productError && !loading && !product) {
    return (
      <div className="container-fluid py-3">
        ...
        <p className="text-muted small mb-3">
          Bu id için kayıtlı bir yedek parça yok: <code className="ms-1">{idOrSlug}</code>
        </p>
    ...
    <div className="container-fluid py-3">
      <div className="mb-3">
        ...
        <p className="text-muted small mb-0">
          Yedek parça temel bilgilerini, stok ve SEO alanlarını buradan yönetebilirsin. Dil seçimi
          DB’den gelir.
        </p>
      </div>
