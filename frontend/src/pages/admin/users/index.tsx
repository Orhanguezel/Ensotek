...
const UsersAdminPage: React.FC = () => {
  ...
  const [setActive, { isLoading: isSettingActive }] =
    useSetUserActiveAdminMutation();
  const [setRoles, { isLoading: isSettingRoles }] =
    useSetUserRolesAdminMutation();
  const [setPassword, { isLoading: isSettingPassword }] =
    useSetUserPasswordAdminMutation();
  const [removeUser, { isLoading: isDeleting }] =
    useRemoveUserAdminMutation();
  ...
  const [originalRole, setOriginalRole] =
    useState<AdminUserRoleName | null>(null);
  ...
  const openEditModal = (user: AdminUserDto) => {
    setFormState({
      ...
      is_active:
        user.is_active === 1 ||
        (user.is_active as any) === true ||
        (user.is_active as any) === "1",
      role: user.role,
      password: "",
    });
  ...
  const handleDelete = async (user: AdminUserDto) => {
    if (
      !window.confirm(
        `"${user.email}" kullanıcısını silmek üzeresin. Devam etmek istiyor musun?`,
      )
    ) {
      return;
    }
                  ...
                  <div className="row g-2">
                    ...
                    <div className="col-md-3">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleFormChange(
                            "role",
                            e.target.value as AdminUserRoleName,
                          )
                        }
                      >
                    ...
                    <div className="col-12">
                      ...
                      <div className="form-text small">
                        Bu alan yalnızca admin tarafından şifre resetlemek
                        için kullanılır. Boş bırakırsan mevcut şifre
                        korunur.
                      </div>
                    </div>
                  </div>
