...
// Ensotek – Site Setting Detail Form (Structured + Raw)
...
const CompanyStructuredRenderer: React.FC<GeneralStructuredRendererProps> = ({
  ...
  const [form, setForm] = useState<CompanyProfileFormState>(() =>
    companyObjToForm(base, { company_name: 'Ensotek', slogan: '', about: '' } as any),
  );

  useEffect(() => {
    setForm(companyObjToForm(base, { company_name: 'Ensotek', slogan: '', about: '' } as any));
  }, [base]);
  ...
  return (
    <CompanyProfileStructuredForm
      ...
      errors={{}}
      disabled={!!disabled}
      seed={{ company_name: 'Ensotek', slogan: '', about: '' } as any}
    />
  );
};
