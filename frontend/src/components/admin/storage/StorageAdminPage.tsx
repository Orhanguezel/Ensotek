...
// Ensotek – Storage Admin Sayfası (Header + Liste + Upload)
// =============================================================
...
export default function StorageAdminPage({
  ...
  const [createAsset, { isLoading: creating }] =
    useCreateAssetAdminMutation();
  const [bulkCreateAssets, { isLoading: bulkCreating }] =
    useBulkCreateAssetsAdminMutation();
  const [deleteAsset, { isLoading: deleting }] =
    useDeleteAssetAdminMutation();
  ...
  const handleBulkFilesChange = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    ...
    try {
      ...
      const successCount = res.items.filter(
        (it) => (it as any).id && !(it as any).error,
      ).length;
      ...
      const errorItems = res.items.filter(
        (it) => (it as any).error,
      ) as any[];

      if (errorItems.length > 0) {
        toast.error(
          `${errorItems.length} dosya yüklenemedi. Detay için console'a bak.`,
        );
