// =============================================================
// FILE: src/components/admin/db/DbAdminHeader.tsx
// Ensotek – Admin DB Backup Header (Snapshot oluştur + Export)
// =============================================================
...
function buildDownloadName(ext: "sql" | "json"): string {
  const d = new Date();
  const stamp = `${d.getFullYear()}${pad(d.getMonth() + 1)}${pad(
    d.getDate(),
  )}_${pad(d.getHours())}${pad(d.getMinutes())}`;
  return `db_backup_${stamp}.${ext}`;
}

export const DbAdminHeader: React.FC<DbAdminHeaderProps> = ({
  onSnapshotCreated,
}) => {
  const [label, setLabel] = useState("");
  const [note, setNote] = useState("");

  const [createSnapshot, { isLoading: isCreating }] =
    useCreateDbSnapshotMutation();
  const [exportSql, { isLoading: isExportingSql }] =
    useExportSqlMutation();
  const [exportJson, { isLoading: isExportingJson }] =
    useExportJsonMutation();
  ...
  const handleCreateSnapshot = async () => {
    try {
      ...
      toast.success(
        `Snapshot oluşturuldu: ${snap.label || snap.filename || snap.id}`,
      );
      setLabel("");
      setNote("");
      onSnapshotCreated?.();
      ...
      const msg =
        err?.data?.error ||
        err?.message ||
        "Snapshot oluşturulurken bir hata oluştu.";
      toast.error(msg);
    }
  };

  const triggerDownload = (blob: Blob, ext: "sql" | "json") => {
  ...
  const handleExportSql = async () => {
    try {
      ...
      triggerDownload(blob, "sql");
      toast.success("SQL yedeği indirildi.");
    } catch (err: any) {
      const msg =
        err?.data?.error ||
        err?.message ||
        "DB export (SQL) sırasında hata oluştu.";
      toast.error(msg);
    }
  };
  ...
  const handleExportJson = async () => {
    try {
      ...
      const msg =
        err?.data?.error ||
        err?.message ||
        "DB export (JSON) sırasında hata oluştu.";
      toast.error(msg);
    }
  };
    ...
    <div className="card mb-3">
      <div className="card-body py-3">
        <div className="d-flex flex-column flex-lg-row gap-3 justify-content-between">
          ...
          <div style={{ minWidth: 0, flex: 2 }}>
            <div className="mb-2">
              ...
              <div className="text-muted small">
                Buradan snapshot noktaları oluşturabilir ve tam veritabanı
                yedeği indirebilirsin.
              </div>
            </div>
          ...
          {/* Sağ: Export butonları */}
          <div
            className="border-start ps-lg-3 ms-lg-3"
          ...
          >
            ...
            <div className="text-muted small mb-2">
              Şu anki veritabanı durumunu tam dump olarak indir.
            </div>
