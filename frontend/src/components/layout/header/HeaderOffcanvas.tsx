...
import { normalizeLocales, normLocaleTag } from "@/i18n/localeUtils";
import { localizePath } from "@/i18n/url";

import { LANG_LABELS, type SupportedLocale } from "@/types/common";
...
const HeaderOffcanvas: React.FC<HeaderOffcanvasProps> = ({
  ...
  // ✅ app_locales GLOBAL olduğu için locale:'*' ile çek
  const { data: appLocalesSetting } = useGetSiteSettingByKeyQuery({
    key: "app_locales",
    locale: "*",
  });

  // ✅ Artık SupportedLocale = string → filtre yok
  const availableLocales = useMemo<SupportedLocale[]>(() => {
    const normalized = normalizeLocales(appLocalesSetting?.value);
    const cleaned = normalized.map((x) => normLocaleTag(x)).filter(Boolean);
    const uniq = Array.from(new Set(cleaned));
    return uniq.length ? uniq : (["tr"] as SupportedLocale[]);
  ...
  const renderMobileMenuItem = (item: MenuItemWithChildren, depth = 0) => {
    ...
    return (
      <li key={id} className={`has-submenu ${isOpen ? "is-open" : ""}`} style={{ paddingLeft: indent }}>
        <button
          type="button"
        ...
        <div className="offcanvas__wrapper">
          <div className="offcanvas__content">
            ...
            <div className="d-flex flex-column gap-2 mb-25">
              <label htmlFor="lang-any" className="d-flex align-items-center gap-2" style={{ fontSize: 14 }}>
              ...
              <div className="d-flex align-items-center gap-2">
                <Link href={loginHref} className="border__btn d-inline-flex align-items-center gap-1" onClick={onClose}>
                  <FiLogIn /> {ui("ui_header_auth", "Login")}
                </Link>

                <Link href={registerHref} className="solid__btn d-inline-flex align-items-center gap-1" onClick={onClose}>
            ...
            <div className="offcanvas__search mb-25">
              <form action="/">
                <input type="text" placeholder={ui("ui_header_search_placeholder", "Search...")} required />
            ...
            <div className="offcanvas__contact mt-30 mb-20">
              ...
              <ul>
                ...
                <li className="d-flex align-items-center">
                  ...
                  <div className="offcanvas__contact-text">
                    <Link href={`mailto:${effectiveBrand.email}`} aria-label={ui("ui_header_email", "Email")}>
            ...
            <div className="offcanvas__social">
              <ul>
                {socialItems.map(({ key, Icon, url }) => (
                  <li key={key}>
                    <Link href={url} target="_blank" rel="noopener noreferrer" aria-label={key} onClick={onClose}>
                      <Icon />
                    </Link>
                  </li>
                ))}
