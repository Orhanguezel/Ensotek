...
const BlogPageContent: React.FC = () => {
  ...
  const sectionSubtitleLabel = ui('ui_blog_sublabel', locale === 'de' ? 'Blog' : 'Blog');
  const sectionTitle = ui('ui_blog_page_title', locale === 'de' ? 'Blog' : 'Blog');
  ...
  const readMore = ui('ui_blog_read_more', locale === 'de' ? 'Devamını oku' : 'Read more');
  const readMoreAria = ui(
    'ui_blog_read_more_aria',
    locale === 'de' ? 'blog detayını görüntüle' : 'view blog details',
  );
  const untitled = ui('ui_blog_untitled', locale === 'de' ? 'Başlıksız blog' : 'Untitled post');
  const emptyText = ui(
    'ui_blog_empty',
    locale === 'de'
  ...
  const blogItems = useMemo(() => {
    ...
    return items.map((p) => {
      ...
      const baseText = (p.meta_description || '').trim() || (p.content_html || '').trim();
