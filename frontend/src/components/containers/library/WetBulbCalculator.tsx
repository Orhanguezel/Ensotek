...
const WetBulbCalculator: React.FC = () => {
    ...
    const tLabel = ui(
        ...
        locale === "tr" ? "Hava Sıcaklığı (°C)" : "Air Temperature (°C)",
    );
    const hLabel = ui(
        "ui_library_wb_humidity_label",
        locale === "tr" ? "Nem Oranı (%)" : "Relative Humidity (%)",
    );
    const btnLabel = ui(
        "ui_library_wb_calculate_button",
        locale === "tr" ? "Hesapla" : "Calculate",
    );
    const heading = ui(
        "ui_library_wb_title",
        locale === "tr"
            ? "Yaş Termometre Sıcaklığı Hesaplayıcı"
    ...
    const resultPrefix = ui(
        ...
        locale === "tr" ? "Yaş termometre sıcaklığı:" : "Wet-bulb temperature:",
    );
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const value = wb.toFixed(2).replace(".", locale === "tr" ? "," : ".");
        setResult(`${value} °C`);
    };
                ...
                <div className="row justify-content-center">
                    <div className="col-xl-8 col-lg-9">
                        ...
                        <div className="wetbulb__result">
                            {error && (
                                <span className="wetbulb__result-error">{error}</span>
                            )}

                            {!error && result && (
                                <>
                                    <span className="wetbulb__result-label">
                                        {resultPrefix}{" "}
                                    </span>
                                    <strong className="wetbulb__result-value">
                                        {result}
                                    </strong>
                                </>
                            )}
