...
// Dekoratif şekiller + placeholder (local)
...
// ✅ Cloudinary placeholders (3)
const HERO_PLACEHOLDERS: Array<{ id: string; src: string; alt: string }> = [
  {
    id: 'ph-1',
    src: 'https://res.cloudinary.com/dbozv7wqd/image/upload/v1752771064/uploads/metahub/gallery/8-1752771062653-586545826.webp',
    alt: 'hero placeholder 1',
  },
  {
    id: 'ph-2',
    src: 'https://res.cloudinary.com/dbozv7wqd/image/upload/v1752771273/uploads/metahub/gallery/1-1752771270894-986291286.webp',
    alt: 'hero placeholder 2',
  },
  {
    id: 'ph-3',
    src: 'https://res.cloudinary.com/dbozv7wqd/image/upload/v1752771255/uploads/metahub/gallery/2-1752771253118-97352961.webp',
    alt: 'hero placeholder 3',
  },
];

// Fallback (DB görseli yoksa buraya düş)
const FALLBACK_PLACEHOLDER_SRC = HERO_PLACEHOLDERS[0].src;
...
const Hero: React.FC = () => {
  ...
  const slides: HeroSlide[] = useMemo(() => {
    ...
    // ✅ Placeholder 3 slide – Cloudinary (metinler i18n fallback)
    if (!active.length) {
      return HERO_PLACEHOLDERS.map((p) => ({
        id: p.id,
        title: '',
        desc: '',
        src: p.src,
    ...
    return active.map<HeroSlide>((s) => {
      ...
      // önce cdn helper → yoksa raw → yoksa placeholder
      const cdn = rawImage ? toCdnSrc(rawImage, 1440, 900, 'fill') : '';
      const src = cdn || rawImage || FALLBACK_PLACEHOLDER_SRC;
  ...
  // CTA metni + link
  const ctaText = current?.buttonText || ui('ui_hero_cta', 'Teklif Al');
  const rawLink = current?.buttonLink || '/contact';
  ...
  return (
    ...
    >
      ...
      <div className="hero__shape-3">
        <Image src={ShapeA} alt="" sizes="(min-width: 1200px) 20vw, 25vw" loading="lazy" />
      </div>
      <div className="hero__shape-4">
        <Image src={ShapeB} alt="" sizes="(min-width: 1200px) 18vw, 22vw" loading="lazy" />
      </div>

      <div className="container-fluid">
        <div className="hero__main-wrapper" style={{ maxWidth: '100%', width: '100%' }}>
          <div className="row g-0 align-items-stretch">
            ...
            <div className="col-12 col-lg-5 d-flex order-1 order-lg-2">
              <div className="hero__thumb-wrapper mb-0 w-100 h-100">
                    ...
                    {slides.map((s, i) => (
                      <SwiperSlide key={s.id}>
                        <div className="project__item" style={{ width: '100%' }}>
                          <div className="projet__thumb w-img" style={{ width: '100%' }}>
                            <Image
                              src={s.src as any}
                              alt={s.alt}
                              width={1440}
                ...
                {slidersLoading && <div className="skeleton-line mt-10" aria-hidden />}
              </div>
            </div>
            ...
            <div className="col-12 col-lg-7 d-flex order-2 order-lg-1">
              ...
              >
                <div className="hero__content" style={{ paddingTop: 32, paddingBottom: 16 }}>
