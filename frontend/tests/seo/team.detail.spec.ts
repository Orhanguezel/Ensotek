...
const slug = (process.env.PLAYWRIGHT_TEAM_SLUG ?? '').trim();

test.describe('SEO: /team/[slug] (detail)', () => {
  test.skip(!slug, 'Set PLAYWRIGHT_TEAM_SLUG to run this test.');

  test('has canonical/og/hreflang ok and JSON-LD parse ok', async ({ page }) => {
    await page.goto(`/team/${encodeURIComponent(slug)}`, { waitUntil: 'domcontentloaded' });
