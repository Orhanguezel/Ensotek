...
const slug = (process.env.PLAYWRIGHT_NEWS_SLUG ?? '').trim();

test.describe('SEO: /news/[slug] (detail)', () => {
  test.skip(!slug, 'Set PLAYWRIGHT_NEWS_SLUG to run this test.');

  test('has canonical/og/hreflang ok and JSON-LD parse ok', async ({ page }) => {
    await page.goto(`/news/${encodeURIComponent(slug)}`, { waitUntil: 'domcontentloaded' });
