...
export function OfferForm({ locale, labels, initialTab, initialPartId, initialPartTitle, initialPartCode, initialProductId, initialProductTitle, initialServiceId, initialServiceName }: Props) {
  ...
  useEffect(() => {
    getServices(API_BASE_URL, { language: locale, is_active: true })
      .then((data) => setServices(Array.isArray(data) ? data : []))
      .catch(() => {});

    getProducts(API_BASE_URL, { language: locale, item_type: 'product', limit: 100 })
      .then((data) => {
    ...
    getProducts(API_BASE_URL, { language: locale, item_type: 'sparepart', limit: 100 })
      .then((data) => {
        const arr = Array.isArray(data) ? data : (data.data ?? []);
        setSpareParts(arr);
      })
