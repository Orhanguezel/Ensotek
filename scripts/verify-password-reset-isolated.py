...
try:
 ...
 env={**os.environ,'DB_HOST':'127.0.0.1','DB_PORT':'3306','DB_NAME':name,'DB_USER':user,'DB_PASSWORD':password,'NODE_ENV':'test','ADMIN_EMAIL':'seed-acceptance@example.invalid','ADMIN_ID':'e5909a46-d405-4710-b05c-01376bb34b51','ADMIN_PASSWORD':secrets.token_hex(24),'JWT_SECRET':secrets.token_hex(32),'COOKIE_SECRET':secrets.token_hex(32),'RESET_CANDIDATE':os.environ.get('RESET_CANDIDATE','/var/www/Ensotek')}
 cwd='/var/www/Ensotek/ensotek_com_tr/backend'
 for command in [['bun','src/db/seed/index.ts','--no-drop','--only=001,002'],['bun','../../scripts/verify-password-reset-boundary.ts']]:
  r=subprocess.run(command,cwd=cwd,env=env,text=True,capture_output=True)
  if r.returncode: print(json.dumps({'step':command[1],'ok':False,'error':r.stderr[:5000]}));raise SystemExit(1)
