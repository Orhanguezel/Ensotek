...
interface Props { id: string; itemType?: ProductItemType; }
...
export default function ProductDetailClient({ id, itemType }: Props) {
  ...
  const { data: item, isFetching, refetch } = useGetProductAdminQuery(
    { id, locale: activeLocale, item_type: itemType }, { skip: isNew },
  );
  const { data: categories = [] } = useListProductCategoriesAdminQuery(
    { locale: activeLocale }, { skip: !activeLocale },
  );
  ...
  const [formData, setFormData] = React.useState({
    locale: activeLocale, title: '', slug: '',
    price: '' as string | number, stock_quantity: '' as string | number,
    product_code: '', description: '', image_alt: '', tags: '',
  ...
  const { data: subcategories = [] } = useListProductSubcategoriesAdminQuery(
    { category_id: formData.category_id, locale: activeLocale }, { skip: !formData.category_id },
  );

  React.useEffect(() => {
    if (item && !isNew) {
      setFormData({
        locale: item.locale || activeLocale, title: item.title || '', slug: item.slug || '',
        ...
        image_url: item.image_url || '', image_asset_id: item.storage_asset_id || '',
  ...
  React.useEffect(() => { if (!isNew && id) refetch(); }, [activeLocale, id, isNew, refetch]);

  const handleBack = () => router.push(backUrl);
  const handleLocaleChange = (next: string) => { setActiveLocale(next); setFormData((p) => ({ ...p, locale: next })); };
  ...
  const handleAIAction = async (action: AIAction) => {
    ...
    const result = await aiAssist({
      title: formData.title, summary: formData.description, content: formData.description,
      tags: formData.tags, locale: activeLocale, target_locales: targetLocales,
      module_key: 'products', action,
    });
    if (!result) return;
    setAiResults(result);
    ...
    if (current) {
      setFormData((prev) => ({
        ...prev, title: current.title || prev.title, slug: current.slug || prev.slug,
  ...
  const handleApplyAILocale = (lc: LocaleContent) => {
    ...
    setFormData((prev) => ({
      ...prev, locale: lc.locale, title: lc.title || '', slug: lc.slug || prev.slug,
      description: lc.content || lc.summary || '',
      meta_title: lc.meta_title || '', meta_description: lc.meta_description || '',
      tags: lc.tags || '',
    }));
  };
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!formData.title.trim()) { toast.error('Baslik zorunludur'); return; }
    const tagsArray = formData.tags ? formData.tags.toString().split(',').map((s) => s.trim()).filter(Boolean) : [];
    const payload = {
      locale: activeLocale, title: formData.title.trim(),
      ...
      product_code: formData.product_code || undefined, description: formData.description || undefined,
      alt: formData.image_alt || undefined, tags: tagsArray,
      category_id: formData.category_id || '', sub_category_id: formData.sub_category_id || null,
      image_url: formData.image_url || null, storage_asset_id: formData.image_asset_id || null,
      is_active: formData.is_active, is_featured: formData.is_featured,
      meta_title: formData.meta_title || undefined, meta_description: formData.meta_description || undefined,
        ...
        <div className="border-b p-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-semibold">{isNew ? 'Yeni Urun' : (item?.title || 'Urun Duzenle')}</div>
                <div className="text-xs text-muted-foreground">{isNew ? 'Yeni urun olustur' : 'Urunu duzenle'}</div>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <AdminLocaleSelect options={localesForSelect} value={activeLocale} onChange={handleLocaleChange} disabled={isLoading} />
              <AIActionDropdown onAction={handleAIAction} loading={aiLoading} disabled={isLoading || !formData.title.trim()} />
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
          <div className="border-b px-3">
            <TabsList className="h-auto bg-transparent p-0">
              <TabsTrigger value="form" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary">Urun</TabsTrigger>
              <TabsTrigger value="specs" disabled={isNew} className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary">
                <ListChecks className="h-4 w-4 mr-1.5" /> Ozellikler
              </TabsTrigger>
              <TabsTrigger value="faqs" disabled={isNew} className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary">
                <HelpCircle className="h-4 w-4 mr-1.5" /> SSS
              </TabsTrigger>
              <TabsTrigger value="reviews" disabled={isNew} className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary">
                <Star className="h-4 w-4 mr-1.5" /> Degerlendirmeler
              </TabsTrigger>
          ...
          <TabsContent value="form" className="p-0">
            <ProductFormTabs formData={formData} setFormData={setFormData}
              handleChange={handleChange} isLoading={isLoading}
              categories={categories} subcategories={subcategories} />
          </TabsContent>

          <TabsContent value="specs" className="p-3">
            {productId ? <ProductSpecsTab productId={productId} locale={activeLocale} disabled={isLoading} /> :
              <div className="py-8 text-center text-sm text-muted-foreground">Once urunu kaydedin.</div>}
          </TabsContent>

          <TabsContent value="faqs" className="p-3">
            {productId ? <ProductFaqsTab productId={productId} locale={activeLocale} disabled={isLoading} /> :
              <div className="py-8 text-center text-sm text-muted-foreground">Once urunu kaydedin.</div>}
          </TabsContent>

          <TabsContent value="reviews" className="p-3">
            {productId ? <ProductReviewsTab productId={productId} disabled={isLoading} /> :
              <div className="py-8 text-center text-sm text-muted-foreground">Once urunu kaydedin.</div>}
          </TabsContent>
...
function ProductFormTabs({ formData, setFormData, handleChange, isLoading, categories, subcategories }: {
  formData: any; setFormData: any; handleChange: any;
  isLoading: boolean; categories: any[]; subcategories: any[];
}) {
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input value={formData.title} onChange={(e) => handleChange('title', e.target.value)} disabled={isLoading} placeholder="Urun basligi" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Slug</Label>
              <Input value={formData.slug} onChange={(e) => handleChange('slug', e.target.value)} disabled={isLoading} placeholder="urun-slug" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="space-y-1.5">
              ...
              <Input type="number" step="0.01" value={formData.price} onChange={(e) => handleChange('price', e.target.value)} disabled={isLoading} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Stok</Label>
              <Input type="number" value={formData.stock_quantity} onChange={(e) => handleChange('stock_quantity', e.target.value)} disabled={isLoading} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Urun Kodu</Label>
              <Input value={formData.product_code} onChange={(e) => handleChange('product_code', e.target.value)} disabled={isLoading} placeholder="SKU-001" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Select value={formData.category_id || 'none'} onValueChange={(v) => { handleChange('category_id', v === 'none' ? '' : v); handleChange('sub_category_id', ''); }} disabled={isLoading}>
                <SelectTrigger><SelectValue placeholder="Kategori sec" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">— Sec —</SelectItem>
                  {categories.map((c: any) => <SelectItem key={c.id} value={String(c.id)}>{c.name || c.slug}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            {formData.category_id && (
              <div className="space-y-1.5">
                ...
                <Select value={formData.sub_category_id || 'none'} onValueChange={(v) => handleChange('sub_category_id', v === 'none' ? '' : v)} disabled={isLoading || subcategories.length === 0}>
                  <SelectTrigger><SelectValue placeholder="Alt kategori sec" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">— Sec —</SelectItem>
                    {subcategories.map((s: any) => <SelectItem key={s.id} value={String(s.id)}>{s.name || s.slug}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            )}
          ...
          <div className="space-y-1.5">
            ...
            <RichContentEditor value={formData.description} onChange={(v) => handleChange('description', v)} disabled={isLoading} />
          </div>

          <div className="flex flex-wrap gap-6">
            <div className="flex items-center gap-2">
              <Switch checked={formData.is_active} onCheckedChange={(v) => handleChange('is_active', v)} disabled={isLoading} />
              <Label className="text-xs cursor-pointer">Aktif</Label>
            </div>
            <div className="flex items-center gap-2">
              <Switch checked={formData.is_featured} onCheckedChange={(v) => handleChange('is_featured', v)} disabled={isLoading} />
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input value={formData.meta_title} onChange={(e) => handleChange('meta_title', e.target.value)} disabled={isLoading} placeholder="SEO basligi" />
            ...
            <div className="space-y-1.5">
              ...
              <Input value={formData.tags} onChange={(e) => handleChange('tags', e.target.value)} disabled={isLoading} placeholder="etiket1, etiket2" />
            </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-xs text-muted-foreground">Meta Aciklama</label>
            <Textarea value={formData.meta_description} onChange={(e) => handleChange('meta_description', e.target.value)} disabled={isLoading} rows={3} placeholder="max 155 karakter" />
