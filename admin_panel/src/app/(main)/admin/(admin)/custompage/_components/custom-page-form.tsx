...
const buildInitialValues = (
  ...
  if (!initial) {
    return {
      id: undefined, page_id: undefined,
      locale: safeLocale, module_key: initialModuleKey.trim(),
      is_published: false, featured: false,
      title: '', slug: '', content: '',
      featured_image: '', featured_image_asset_id: '', featured_image_alt: '',
      summary: '', meta_title: '', meta_description: '', tags: '',
      images: [], storage_image_ids: [],
    };
  }

  const tagsString = Array.isArray(initial.tags) && initial.tags.length
    ? initial.tags.join(', ') : (initial.tags_raw ?? '');

  return {
    id: initial.id, page_id: initial.id,
    locale: getLocaleFromDto(initial, safeLocale),
    module_key: (initial as any).module_key ?? initialModuleKey ?? '',
    is_published: !!initial.is_published, featured: !!initial.featured,
...
export const CustomPageForm: React.FC<CustomPageFormProps> = ({
  mode, initialData, initialModuleKey, loading, saving,
  locales, localesLoading, defaultLocale, onLocaleChange, onSubmit, onCancel,
}) => {
  const t = useAdminT();
  ...
  const handleChange = (field: keyof CustomPageFormValues) =>
    (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      setValues((prev) => ({ ...prev, [field]: e.target.value }) as CustomPageFormValues);
    };

  const handleCheckboxChange = (field: keyof CustomPageFormValues) =>
  ...
  const handleLocaleChange = async (nextLocaleRaw: string) => {
    ...
    if (norm(values.locale) === nextLocale) { onLocaleChange?.(nextLocale); return; }
    onLocaleChange?.(nextLocale);
    if (mode === 'create' || !initialData) { setValues((prev) => ({ ...prev, locale: nextLocale })); return; }
    const baseId = initialData.id;
    if (!baseId) { setValues((prev) => ({ ...prev, locale: nextLocale })); return; }
  ...
  const handleAIAction = async (action: AIAction) => {
    ...
    const result = await aiAssist({
      ...
      tags: values.tags, locale: values.locale || 'de',
      target_locales: targetLocales, module_key: values.module_key, action,
    });
    if (!result) return;
    setAiResults(result);
    ...
    if (current) {
      setValues((prev) => ({
        ...prev,
        title: current.title || prev.title, slug: current.slug || prev.slug,
        summary: current.summary || prev.summary, content: current.content || prev.content,
  ...
  const applyAILocale = React.useCallback((locale: string) => {
    ...
    setValues((prev) => ({
      ...prev, locale,
      title: match.title || '', slug: match.slug || prev.slug,
      summary: match.summary || '', content: match.content || '',
      meta_title: match.meta_title || '', meta_description: match.meta_description || '',
      tags: match.tags || '',
    }));
  }, [aiResults]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    void onSubmit({
      ...
      summary: values.summary.trim(), meta_title: values.meta_title.trim(),
      meta_description: values.meta_description.trim(), tags: values.tags.trim(),
    });
  };

  const localeSelectOptions = useMemo(
    () => (locales ?? []).map((x) => ({ value: norm(x.value), label: x.label })), [locales],
  );

  const imageMetadata = useMemo<Record<string, string | number | boolean>>(
    () => ({
      module_key: 'custom_pages', locale: values.locale || safeDefaultLocale,
  ...
  const handleAddManualImage = () => {
    ...
    if (!url) { toast.error(t('admin.customPage.form.invalidImageUrl')); return; }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="rounded-lg border bg-card">
        ...
        <div className="border-b p-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground">{t('admin.customPage.form.formDescription')}</div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {/* Form/JSON toggle */}
              <div className="inline-flex overflow-hidden rounded-md border">
                <button type="button" className={`px-3 py-1 text-xs ${activeMode === 'form' ? 'bg-muted font-semibold' : 'bg-background'}`}
                  onClick={() => setActiveMode('form')} disabled={disabled}>Form</button>
                <button type="button" className={`px-3 py-1 text-xs ${activeMode === 'json' ? 'bg-muted font-semibold' : 'bg-background'}`}
                  onClick={() => setActiveMode('json')} disabled={disabled}>JSON</button>
              </div>
              {onCancel && <button type="button" className="rounded-md border px-3 py-1 text-xs" onClick={onCancel} disabled={disabled}>{t('admin.customPage.form.back')}</button>}
              <AIActionDropdown onAction={handleAIAction} loading={aiLoading} disabled={disabled || !values.title.trim()} />
              {/* Save */}
              <button type="submit" className="rounded-md bg-primary px-3 py-1 text-xs text-primary-foreground disabled:opacity-60" disabled={disabled}>
                {saving ? (mode === 'create' ? t('admin.customPage.form.creating') : t('admin.customPage.form.saving'))
                  : (mode === 'create' ? t('admin.customPage.form.createBtn') : t('admin.customPage.form.saveBtn'))}
              </button>
        ...
        <div className="p-3">
          {activeMode === 'json' ? (
            <AdminJsonEditor value={values} disabled={disabled}
              onChange={(next) => setValues(next as CustomPageFormValues)}
              label="Custom Page JSON" helperText={t('admin.customPage.form.jsonHelperText')} />
          ) : (
            <>
              {/* Dil + Modul secici */}
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end">
                <div className="flex-1">
                  <AdminLocaleSelect value={values.locale} onChange={handleLocaleChange}
                    options={localeSelectOptions as any} loading={!!localesLoading}
                ...
                <div className="w-full sm:w-48">
                  <label className="mb-1.5 block text-xs font-medium text-muted-foreground">Modul</label>
                  <select className="h-9 w-full rounded-md border bg-background px-3 text-sm"
                    value={values.module_key} onChange={(e) => setValues((p) => ({ ...p, module_key: e.target.value }))} disabled={disabled}>
              ...
              <FormTabs values={values} setValues={setValues} disabled={disabled}
                slugTouched={slugTouched} setSlugTouched={setSlugTouched}
                handleChange={handleChange} handleCheckboxChange={handleCheckboxChange}
                imageMetadata={imageMetadata} contentImageSize={contentImageSize}
                setContentImageSize={setContentImageSize} contentImagePreview={contentImagePreview}
                handleAddContentImage={handleAddContentImage}
                manualImageUrl={manualImageUrl} manualImageAlt={manualImageAlt}
                setManualImageUrl={setManualImageUrl} setManualImageAlt={setManualImageAlt}
      ...
      {aiResults && aiResults.length > 1 && (
        <div className="rounded-lg border border-purple-200 bg-transparent dark:border-purple-800">
          <AIResultsPanel results={aiResults} currentLocale={values.locale}
            onApply={(lc) => { applyAILocale(lc.locale); onLocaleChange?.(lc.locale); }}
...
function FormTabs({ values, setValues, disabled, slugTouched, setSlugTouched, handleChange, handleCheckboxChange,
  imageMetadata, contentImageSize, setContentImageSize, contentImagePreview, handleAddContentImage,
  manualImageUrl, manualImageAlt, setManualImageUrl, setManualImageAlt, handleAddManualImage,
}: {
  values: CustomPageFormValues; setValues: React.Dispatch<React.SetStateAction<CustomPageFormValues>>;
  disabled: boolean; slugTouched: boolean; setSlugTouched: React.Dispatch<React.SetStateAction<boolean>>;
  handleChange: (field: keyof CustomPageFormValues) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void;
  handleCheckboxChange: any; imageMetadata: Record<string, any>;
  contentImageSize: any; setContentImageSize: any; contentImagePreview: any; handleAddContentImage: any;
  manualImageUrl: string; manualImageAlt: string; setManualImageUrl: any; setManualImageAlt: any; handleAddManualImage: any;
}) {
  ...
  return (
    <div className="space-y-3">
      ...
      {tab === 'content' && (
        <CustomPageMainColumn values={values} disabled={disabled} slugTouched={slugTouched}
          setSlugTouched={setSlugTouched} setValues={setValues} handleChange={handleChange}
          handleCheckboxChange={handleCheckboxChange} />
      )}

      {tab === 'images' && (
        <div className="space-y-4">
          <CustomPageFormImageColumn metadata={imageMetadata} disabled={disabled}
          ...
          <CustomPageSidebarColumn values={values} disabled={disabled} imageMetadata={imageMetadata}
            contentImageSize={contentImageSize} setContentImageSize={setContentImageSize}
            contentImagePreview={contentImagePreview} handleAddContentImage={handleAddContentImage}
            manualImageUrl={manualImageUrl} manualImageAlt={manualImageAlt}
            setManualImageUrl={setManualImageUrl} setManualImageAlt={setManualImageAlt}
            handleAddManualImage={handleAddManualImage} setValues={setValues} />
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <input type="text" className="h-8 w-full rounded-md border bg-background px-3 text-sm"
                value={values.meta_title} onChange={handleChange('meta_title')} disabled={disabled} placeholder="Sayfa basligi (SEO)" />
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" className="h-8 w-full rounded-md border bg-background px-3 text-sm"
                value={values.tags} onChange={handleChange('tags')} disabled={disabled} placeholder="etiket1, etiket2" />
            </div>
          </div>
          <div className="space-y-1.5">
            ...
            <textarea className="w-full rounded-md border bg-background px-3 py-2 text-sm" rows={3}
              value={values.meta_description} onChange={handleChange('meta_description')} disabled={disabled}
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-muted-foreground">Kapak Gorseli Alt Metni</label>
            <input type="text" className="h-8 w-full rounded-md border bg-background px-3 text-sm"
              value={values.featured_image_alt} onChange={handleChange('featured_image_alt')} disabled={disabled}
