  ...
  if (loading) {
    return (
      <div className="flex min-h-75 items-center justify-center rounded-lg border bg-card">
        <span className="animate-pulse text-muted-foreground text-sm">{t("loading")}</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border bg-card p-4">
        <div className="mb-3 font-medium text-sm">{t("distributionTitle", { total: String(totalRequests) })}</div>
        <ChartContainer config={chartConfig} className="h-70 w-full">
          <BarChart data={topCountries} margin={{ top: 4, right: 8, left: 0, bottom: 40 }}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-border" />
            <XAxis dataKey="code" tick={{ fontSize: 11 }} angle={-45} textAnchor="end" interval={0} height={50} />
      ...
      {/* Top Countries Table */}
      <div className="rounded-lg border bg-card p-4">
        <div className="mb-2 font-medium text-sm">{t("topCountries")}</div>
