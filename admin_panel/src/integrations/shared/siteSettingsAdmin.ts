...
import type { SettingValue, SiteSetting, SiteSettingRow, SiteSettingLike } from './siteSettings';
...
export const SITE_SETTINGS_SMTP_KEYS: readonly string[] = [
  'smtp_host', 'smtp_port', 'smtp_ssl', 'smtp_username',
...
export function mapSiteSettingsToSmtpForm(settings: (SiteSetting | SiteSettingRow)[] | undefined): SiteSettingsSmtpForm {
...
export const SITE_SETTINGS_CLOUDINARY_KEYS: readonly string[] = [
  ...
  'storage_cdn_public_base', 'storage_public_api_base',
  'cloudinary_cloud_name', 'cloudinary_api_key', 'cloudinary_api_secret',
  'cloudinary_folder', 'cloudinary_unsigned_preset',
] as const;
...
export function mapSiteSettingsToCloudinaryForm(settings: (SiteSetting | SiteSettingRow)[] | undefined): SiteSettingsCloudinaryForm {
...
export function buildSiteSettingsCloudinaryUpdates(form: SiteSettingsCloudinaryForm): { key: string; value: SettingValue }[] {
  return Object.entries(form).map(([key, value]) => ({ key, value }));
}
...
export const EMPTY_SITE_SETTINGS_BRANDING_FORM: SiteSettingsBrandingForm = {
  app_name: '', app_copyright: '', html_lang: '', theme_color: '',
  og_image: '', favicon_16: '', favicon_32: '', apple_touch_icon: '',
  meta_title: '', meta_description: '', og_title: '', og_description: '',
  og_url: '', twitter_card: '',
};

export const SITE_SETTINGS_BRANDING_PLACEHOLDER_KEYS: Record<keyof SiteSettingsBrandingForm, string> = {
  app_name: 'appNamePlaceholder', app_copyright: 'appCopyrightPlaceholder',
  html_lang: 'htmlLangPlaceholder', theme_color: 'themeColorPlaceholder',
  og_image: 'ogImagePlaceholder', favicon_16: 'favicon16Placeholder',
  favicon_32: 'favicon32Placeholder', apple_touch_icon: 'appleTouchIconPlaceholder',
  meta_title: 'metaTitlePlaceholder', meta_description: 'metaDescriptionPlaceholder',
  og_title: 'ogTitlePlaceholder', og_description: 'ogDescriptionPlaceholder',
  og_url: 'ogUrlPlaceholder', twitter_card: 'twitterCardPlaceholder',
};

export function normalizeSiteSettingsBrandingConfig(value: unknown): Record<string, unknown> {
  if (!value) return {};
  if (typeof value === 'string') {
    try { const p = JSON.parse(value); return typeof p === 'object' && p ? p : {}; }
    catch { return {}; }
  }
...
export function brandingToSiteSettingsForm(config: unknown): SiteSettingsBrandingForm {
  ...
  return {
    app_name: pick('app_name'), app_copyright: pick('app_copyright'),
    html_lang: pick('html_lang'), theme_color: pick('theme_color'),
    og_image: pick('og_image'), favicon_16: pick('favicon_16'),
    favicon_32: pick('favicon_32'), apple_touch_icon: pick('apple_touch_icon'),
    meta_title: pick('meta_title'), meta_description: pick('meta_description'),
    og_title: pick('og_title'), og_description: pick('og_description'),
    og_url: pick('og_url'), twitter_card: pick('twitter_card'),
  };
}
...
export function normalizeSiteSettingsLocaleRows(value: unknown): SiteSettingsLocaleRow[] {
  ...
  if (Array.isArray(value)) raw = value;
    ...
    try { const p = JSON.parse(value); if (Array.isArray(p)) raw = p; } catch { /* noop */ }
...
export function toStructuredObjectSeed<T extends Record<string, unknown>>(
  value: unknown,
  seed: T,
): T {
  let parsed: unknown = value;
  if (typeof parsed === 'string') {
    try { parsed = JSON.parse(parsed); } catch { return { ...seed }; }
  }
...
export const SITE_SETTINGS_SOCIAL_KEYS: readonly string[] = [
  'instagram', 'facebook', 'linkedin', 'youtube', 'x', 'github', 'tiktok', 'pinterest',
] as const;
