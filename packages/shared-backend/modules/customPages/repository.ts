...
import { normalizeLocaleStr, toBool, extractHtmlFromJson, packContent } from '../_shared';
...
function normalizeContent(s: string | null): string | null {
  if (!s) return s;
  const extracted = extractHtmlFromJson(s);
  return extracted || s;
}

function pickLocalizedRow<T extends PageRow>(rows: T[], locale: string): T | null {
  if (!rows.length) return null;
  const row = rows.find((r) => r.locale === locale) ?? rows.find((r) => r.locale === 'tr') ?? rows[0] ?? null;
  if (!row) return null;
  return { ...row, content: normalizeContent(row.content) };
}
...
export async function repoCreateCustomPage(data: CreateCustomPageInput) {
  ...
  await db.insert(customPagesI18n).values({
    ...
    title: data.title,
    slug: data.slug,
    content: packContent(data.content ?? ''),
...
export async function repoUpdateCustomPage(id: string, data: UpdateCustomPageInput) {
  ...
  if (data.featured_image !== undefined) {
    pagePatch.featured_image = data.featured_image;
    pagePatch.image_url = data.featured_image;
  }
  if (data.image_url !== undefined) pagePatch.image_url = data.image_url;
  ...
  if (data.content !== undefined) i18nPatch.content = packContent(data.content ?? '');
