...
// MOE Kompozit – Offer Module Service
...
function renderPricingBlock(input: OfferMailContext): string {
  const rows = [
    ...
    ['KDV Oranı', input.vat_rate ?? '-'],
    ['KDV Toplamı', input.vat_total ?? '-'],
    ['Sevkiyat', input.shipping_total ?? '-'],
    ['Genel Toplam', input.gross_total ?? '-'],
    ['Geçerlilik', input.valid_until ?? '-'],
  ]
...
async function getOfferMailSiteName(locale?: string | null): Promise<string> {
  const raw = await getSiteSettingValue('site_title', locale);
  return typeof raw === 'string' && raw.trim() ? raw.trim() : 'MOE Kompozit';
}

async function sendKompozitOfferCustomerMail(input: OfferMailContext) {
  const siteName = await getOfferMailSiteName(input.locale);
  const subject = `${siteName} teklifiniz hazır`;
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#111827">
      <h2>Teklifiniz hazır</h2>
      <p>Merhaba ${escapeMailHtml(input.customer_name)},</p>
      <p>${escapeMailHtml(siteName)} ekibi teklif kaydınızı hazırladı.</p>
      ${renderPricingBlock(input)}
      ${
        input.pdf_url
          ? `<p style="margin-top:16px"><a href="${escapeMailHtml(input.pdf_url)}">Teklif dosyasını görüntüle</a></p>`
          : ''
      }
    </div>
  `;

  await sendMailRaw({ to: input.email, subject, html });
}

async function sendKompozitOfferAdminMail(input: OfferMailContext, to: string) {
  const siteName = await getOfferMailSiteName(input.locale);
  const subject = `[${siteName}] Teklif gönderildi`;
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#111827">
      <h2>Müşteriye teklif gönderildi</h2>
      <p><strong>Müşteri:</strong> ${escapeMailHtml(input.customer_name)}</p>
      <p><strong>E-posta:</strong> ${escapeMailHtml(input.email)}</p>
      <p><strong>Teklif No:</strong> ${escapeMailHtml(input.offer_no || input.offer_id || '-')}</p>
      ${renderPricingBlock(input)}
      ${
        input.pdf_url
          ? `<p style="margin-top:16px"><a href="${escapeMailHtml(input.pdf_url)}">PDF bağlantısı</a></p>`
          : ''
      }
    </div>
  `;
...
async function sendCustomerOfferMail(ctx: OfferEmailContext): Promise<boolean> {
  ...
  await sendKompozitOfferCustomerMail({
...
async function sendAdminOfferMail(ctx: OfferEmailContext): Promise<boolean> {
  ...
  for (const to of adminEmails) {
    await sendKompozitOfferAdminMail(
      {
        customer_name: o.customer_name,
        company_name: o.company_name,
        offer_no: o.offer_no ?? o.id,
...
Teklif ID: ${offer.id}`;
  ...
  try {
    ...
    for (const to of adminEmails) {
      await sendKompozitOfferRequestAdminMail(
        {
