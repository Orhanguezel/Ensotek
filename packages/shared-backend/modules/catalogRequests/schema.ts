...
export const leadCatalogDownloads = mysqlTable(
  'lead_catalog_downloads',
  {
    ...
    email_verified_at: timestamp('email_verified_at', { fsp: 3 }),
    verification_token_hash: varchar('verification_token_hash', { length: 64 }),
    verification_expires_at: timestamp('verification_expires_at', { fsp: 3 }),
    failure_reason: text('failure_reason'),
    last_attempt_at: timestamp('last_attempt_at', { fsp: 3 }),
  ...
  (t) => [
    ...
    index('lead_catalog_verification_token_idx').on(t.verification_token_hash),
